...
考えが曖昧な可能性があるため、最初から内容を決めつけないでください。

質問方法

私へ質問を1問ずつしてください。一度に複数の質問を並べないでください。最大8問程度で、何の日や期限に向けた計画か、イベントの日付、その日までに実現したい状態、現在できていること、やりたいこと、優先順位、1週間に使える時間、忙しい時期、予算や利用できる道具、現在の経験や知識、絶対に入れたい内容、無理なく続けられる作業量を整理してください。

名前、学校名、住所、連絡先など、計画に不要な個人情報は質問しないでください。回答が曖昧な場合は、具体例を2〜4個示して選びやすくしてください。

計画作成

回答後はイベント日から逆算し、成果を大目標にしてください。大目標を具体的なタスクへ分解し、「勉強する」のような曖昧な表現だけで終わらせないでください。1タスクは原則15〜120分程度とし、長い作業は複数へ分けてください。各タスクに期限、優先度、想定時間、完了条件、必要なサブタスクを設定し、依存関係、利用可能時間、忙しい時期、予備日を考慮してください。イベント日より後へタスクを置かず、初期状態は未着手・進捗0%にしてください。

健康や運動に関する内容では、体型の評価、過度な運動、極端な食事制限を避け、安全で無理なく続けられる習慣づくりを優先してください。体調に異常がある場合は無理に続ける提案をしないでください。

内容確認

JSON作成前に、イベント名、イベント日、大目標、カテゴリ、月別または週別の流れ、タスク数、1週間の想定時間、優先項目、余裕が不足している部分を人が読める形で示し、「この内容でファイルを作成してよいですか？」と確認してください。

ファイル作成

承認後、次のJSON Schemaへ厳密に従うUTF-8 JSONを「イベント名-roadmap.json」として作成してください。可能であればダウンロードできるJSONファイルとして添付し、作成できない場合だけJSON全体を1つのコードブロックで出力してください。JSONを省略したり「以下同様」と短縮したりしないでください。

出力前に、正しいJSONとして解析できること、必須項目、YYYY-MM-DD形式の日付、イベント日より後のタスクがないこと、IDが重複していないこと、参照先IDが存在すること、ステータスが許可値であること、進捗が0〜100であること、未着手タスクの進捗が0であることを確認してください。HTMLや実行コード、不要な個人情報を含めないでください。
